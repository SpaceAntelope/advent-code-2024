...
type LastPosition = Cycle of Point | OutOfBounds of Point
...
let traverse (matrix: char array2d) (pos: Point) (dir: Dir) =
    ...
    let isObstacle pos = matrix.[fst pos, snd pos] = '#'
    ...
    while isInBounds currentPos && isNotVisited currentPos currentDir do
        // && isNotVisited current dir do 
...
let traceCycles matrix (originalPath : (Point*Dir) list)=
    let nextObstacleIndex = pos2obsIndex matrix
    let rows,cols = matrix |> Global.matrixSize

    let obstacleLeadsToCycle newObstacle dir =
        let updatedObstacleIndex = 
            obstacleNeighborhood matrix newObstacle 
            |> List.fold (fun (state: Map<(Point * Dir),(int * int)>) (cell,obs) -> state |> Map.add cell obs ) nextObstacleIndex

        let mutable visited = Set.empty<Point*Dir>
        let isNotVisited pos dir = visited.Contains (pos,dir) |> not
        let isNotObstacle pos dir = updatedObstacleIndex.ContainsKey (pos,dir) |> not
        let isInBounds (row,col) = row <= rows-1 || col <= cols-1 || col >= 0 || row >= 0

        let traceSimplePath pos dir =
            let mutable current = pos
            [
                while 
                    isNotObstacle current dir 
                    && isInBounds current do
                    // && isNotVisited current dir do 
                    
                    yield current,dir
                    let nextPosition = next current dir
                    //visited <- visited |> Set.add (nextPosition, dir)
        ...
        let rec trace (currentPosition) (currentDirection) =
            ...
            | None -> 
                false
                //seq { yield! traceStraightPathUntilObstacleOrBoundary currentPosition currentDirection matrix } |> Seq.map (fun x -> x,currentDirection)
            | Some obstacleAhead -> 
                let lastPosition = positionWhenFacingObstacle obstacleAhead currentDirection
                let nextDirection = rotateRight currentDirection
                let path = traceSimplePath currentPosition currentDirection                    
                // pathBetweenPointsExcludeLast currentPosition nextPosition |> Seq.map (fun x -> x, currentDirection)
                let isCycle = path |> List.tryFind (fun dirpoint -> visited |> Set.contains dirpoint ) |> Option.isSome
   
                visited <- 
                    path 
                    |> List.fold (fun state (point,dir) -> visited |> Set.add (point,dir)) visited
                
                isCycle || trace lastPosition nextDirection
   

        let initPos= positionWhenFacingObstacle newObstacle dir
        let initDir = rotateRight dir
        trace initPos initDir    

    originalPath
    |> List.skip 1
    |> List.filter (fun (pos,dir) -> obstacleLeadsToCycle pos dir)
    |> List.length
